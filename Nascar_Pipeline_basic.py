...
def compute_driver_features(
    ...
    for (driver, team), grp in df.groupby(["driver", "team_name"], sort=False):
        ...
        for idx in range(len(grp)):
            ...
            ytd_prior = prior[prior["race_id"] >= year_start_race]
...
def generate_matchups(
    ...
    """
    value_cols = ["momentum"] + [f"{f}_baseline" for f in feature_cols] + [f"{f}_track" for f in feature_cols]# + [f"{f}_ytd" for f in feature_cols]
    diff_cols  = ["diff_momentum"] + [f"diff_{f}_baseline" for f in feature_cols] + [f"diff_{f}_track" for f in feature_cols]# + [f"diff_{f}_ytd" for f in feature_cols]
...
def run_pipeline(
    ...
    print("Features after filtering:", X_train.shape[1], X_train.columns.tolist())
...
if __name__ == "__main__":
    ...
    driver_features.to_csv("./driver_features_basic.csv", index=False)
    matchups.to_csv("./matchups_basic.csv", index=False)
    print("Outputs saved to driver_features_basic.csv and matchups_basic.csv")