    ...
    for (driver, team), grp in df.groupby(["driver", "team_name"], sort=False):
        ...
        for idx in range(len(grp)):
            ...
            if len(ytd_prior) < 10:
                continue
            if row["year"] > 2022:
                if len(prior[prior["year"] == row["year"] - 1]) < 10:
                    continue
                else:
...
if __name__ == "__main__":
    ...
    driver_features.to_csv("./driver_features_paper.csv", index=False)
    matchups.to_csv("./matchups_paper.csv", index=False)
    print("Outputs saved to driver_features_paper.csv and matchups_paper.csv")